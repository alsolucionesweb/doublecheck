        ...
        <div class="card-body">
            <nav class="navbar navbar-expand-lg navbar-warning bg-warning mb-4">
                <div class="navbar-brand">    
                    <h5 class="card-title">
                        <i class="fa-solid fa-chart-line"></i> Analisis de campañas por indicadores
                    </h5>                            
                </div>
                <div class="container-fluid justify-content-end">
                    <div class="d-flex flex-wrap gap-3">
                        <!-- Filtro de Semana (Dropdown tipo radio) -->
                        <div class="dropdown">
                            <button class="btn btn-outline-dark dropdown-toggle" type="button" data-bs-toggle="dropdown">
                                Semana: {{ $semanas->firstWhere('idSemana', $idSemana)->idSemana ?? 'Seleccionar' }}
                            </button>
                            <ul class="dropdown-menu p-2">
                                @foreach($semanas as $sem)
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input filtro-semana" type="radio"
                                                name="semana" id="semana_{{ $sem->idSemana }}"
                                                value="{{ $sem->idSemana }}"
                                                {{ $sem->idSemana == $idSemana ? 'checked' : '' }}>
                                            <label class="form-check-label" for="semana_{{ $sem->idSemana }}">
                                                Semana {{ $sem->idSemana }}
                                            </label>
                                        </div>
                                    </li>
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu p-2" style="max-height: 300px; overflow-y: auto;">
                                @foreach($candidatos as $candidato)
                                    <li>
                                        <div class="form-check">
                                            <input class="form-check-input filtro-candidato" type="checkbox" value="{{ $candidato->name }}" id="candidato_{{ $candidato->id }}">
                ...
                <tbody class="table-group-divider">
                    @foreach($indicadores as $indicador)
                        <tr>
                            ...
                            @foreach($candidatos as $candidato)
                                @php
                                    $clave = $indicador->id . '-' . $candidato->id;
...
$(document).ready(function () {
    ...
    $('.filtro-semana').on('change', function () {
        ...
        url.searchParams.set('semana', semanaSeleccionada);
        window.location.href = url.toString(); // redirige
    });

    $('.filtro-candidato').on('change', function () {
        const seleccionados = $('.filtro-candidato:checked').map(function () {
            return $(this).val();
        }).get();
